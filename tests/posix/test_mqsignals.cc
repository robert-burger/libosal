...
using testutils::wait_nanoseconds;
  ...
  namespace test_interrupts {
    ...
    void handle_test_signal(int s){
      printf("received signal %i\n", s);
    }

    TEST(MessageQueue, TestInterruptSend) {
    ...
    TEST(MessageQueue, TestInterruptTimedSend) {
	...
	orv = osal_mq_timedsend(&mqueue, (const osal_char_t*)&buf, sizeof(buf), prio, &deadline);
    ...
    TEST(MessageQueue, TestInterruptReceive) {
    ...
    TEST(MessageQueue, TestInterruptTimedReceive) {
	...
	osal_timer_t deadline = set_deadline(2, 0);
	orv = osal_mq_timedreceive(&mqueue, (osal_char_t*)&buf, sizeof(buf), &prio, &deadline);
	// we expect this to fail with timeout, because of is ignored
	EXPECT_EQ(orv, OSAL_ERR_TIMEOUT) << "osal_mq_send() failed";
