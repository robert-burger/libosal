...
namespace test_condvar {
  ...
  using testutils::popcount;
	...
	int get_free_mask(std::atomic<ulong> *active_mask,
			       int min_number,
			       int thread_number){
...
namespace condvar_multithread {
  ...
  void* test_condvar_count(void *arg)
  {
    ...
    while (true){
      ...
      while (!((p_shared_objects->active_mask >> thread_id) & 1ul)
	     && (!p_shared_objects->stop)){
	      ...
	      EXPECT_EQ(orv, OSAL_OK) <<
		      "error in receiver: osal_condvar_wait()";
      }
	 ...
	 EXPECT_EQ(orv, OSAL_OK) <<
		 "error in receiver: osal_mutex_unlock()";
	 break;
      }
      ...
      std::atomic_fetch_and(&p_shared_objects->active_mask,
			    clear_mask);
  ...
  TEST(Condvar, ParallelMasked)
    {
      ...
      memset(shared_objects.event_count, 0,
	     sizeof(shared_objects.event_count));
      // start threads
...
//	      ASSERT_EQ(rv, 0) << "pthread_create() failed";	
      ...
      for (int i=0; i < LOOPCOUNT1; i++){
	      ...
	      std::atomic_fetch_or(&shared_objects.active_mask,
				   (1ul << selected_thread));
      ...
      ASSERT_EQ(orv, OSAL_OK) <<
	      "stopping: osal_mutex_lock() failed";
      // instruct threads to stop
      shared_objects.stop = true;
      // broadcast change
      orv = osal_condvar_broadcast(&shared_objects.condvar);
      ASSERT_EQ(orv, OSAL_OK) <<
	      "stopping: osal_condvar_broadcast() failed";
      orv = osal_mutex_unlock(&shared_objects.mutex);
      ASSERT_EQ(orv, OSAL_OK) <<
	      "stopping: osal_mutex_unlock() failed";
