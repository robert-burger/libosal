...
TEST(TimerSleep, SaneSingleThreaded)
{
  ...
  check_sleep_times(req_wait_times,
		    runs_realtime,
		    TIMER_TOLERANCE_LESS_NS,
		    TIMER_TOLERANCE_MORE_NS);	
}
...
*/
  ...
  void* check_sleep_times_mt(void* vparams)
  {
    ...
    check_sleep_times(params.wait_times,
		      params.runs_realtime,
		      params.less_margin,
		      params.more_margin);	
    return nullptr;
...
TEST(TimerSleep, SaneMultiThreaded)
{
  ...
  for (int i = 0; i < N_THREADS; i++){
    ...
    vector<osal_uint64_t> shuffled_times = shuffle_vector(req_wait_times,
							  seed_value);
    // store parameters
    const sleep_params_t params = {shuffled_times,
				  runs_realtime,
				  TIMER_TOLERANCE_LESS_NS,
				  TIMER_TOLERANCE_MORE_NS};
    thread_params_vec.push_back(params);
...
TEST(TimerSleepUntil, SaneSingleThreaded)
{
  ...
  check_sleep_until(req_wait_times,
		    runs_realtime,
		    TIMER_TOLERANCE_LESS_NS,
		    TIMER_TOLERANCE_MORE_NS);	
}
  ...
  void* check_sleep_until_mt(void* vparams)
  {
    sleep_params_t * p_params = (sleep_params_t *) vparams;
    sleep_params_t params  = *p_params;
    check_sleep_until(params.wait_times,
		      params.runs_realtime,
		      params.less_margin,
		      params.more_margin);	
    return nullptr;
