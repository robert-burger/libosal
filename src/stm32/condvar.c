 ...
 * \date 07 Aug 2022
 *
 * \brief OSAL stm32 condvar source.
 *
 * OSAL stm32 condvar source.
 */
...
osal_retval_t osal_condvar_wait(osal_condvar_t *cv, osal_mutex_t *mtx) {
    assert(cv != NULL);
    //pthread_cond_wait(&cv->stm32_cond, &mtx->stm32_mtx);
    return OSAL_OK;
}
...
//
//    do {
//        local_ret = pthread_cond_timedwait(&cv->posix_cond, &mtx->stm32_mtx, &ts);
